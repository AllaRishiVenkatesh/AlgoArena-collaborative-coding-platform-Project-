  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      if (response.data.username) {
        ...
        toast.warn("Login successful but username missing.");
      }
    } catch (error) {
      ...
      if (error.response) {
        ...
        if (errorMessage.toLowerCase().includes("user not found")) {
          toast.error("New user? Sign up first!", { position: "top-right", autoClose: 3000 });
        } else {
      ...
      <div className="z-10 w-full max-w-md px-6">
        <div className="bg-slate-900/80 backdrop-blur-md border border-slate-800 rounded-xl shadow-xl p-8">
          <div className="text-center mb-8">
            ...
            <h2 className="text-2xl font-bold tracking-tight text-slate-200 mb-2">Welcome Back</h2>
            <p className="text-slate-400 text-sm">Enter your credentials to access the arena</p>
          </div>
